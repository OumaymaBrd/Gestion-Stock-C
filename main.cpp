...
bool verifier_alphanumerique(char name[]) {
    for (int j = 0; j < strlen(name); j++) {
        if (!isalnum(name[j]) && name[j] != ' ') {
            printf(c31 "\n\n\tErreur : Le caractère '%c' n'est pas valide.\n\n" c0, name[j]);
            return false;
        }
    }
    return true;
...
bool verifier_numero_tel(char name[]) {
    if (name[0] != '+') {
        printf(c31 "\n\n\tErreur : Le numéro doit commencer par '+'.\n\n" c0);
        return false;
    }
...
void saisir_structure(Contact *C, int nb_saisir, int nb_total_contacts) {
    for (int i = 0; i < nb_saisir; i++) {
        int index = nb_total_contacts + i;  // Insertion à partir du dernier contact saisi

        // Saisie du nom
        while (1) {
            printf("\n\n\t Entrer les informations de Contact %d ", index + 1);
            printf("\n\t\tEntrez le nom du contact : ");
            fgets(C->Nom[index], 500, stdin);
            C->Nom[index][strcspn(C->Nom[index], "\n")] = '\0';  // Supprimer le retour à la ligne

            if (verifier_alphanumerique(C->Nom[index])) break;
        ...
        while (1) {
            ...
            fgets(C->tel[index], 20, stdin);
            C->tel[index][strcspn(C->tel[index], "\n")] = '\0';

            if (verifier_numero_tel(C->tel[index])) break;
        ...
        while (1) {
            ...
            fgets(C->email[index], 500, stdin);
            C->email[index][strcspn(C->email[index], "\n")] = '\0';

            if (isValidEmail(C->email[index])) break;
...
void modifier_contact(Contact *C, int nb_total_contacts) {
    if (nb_total_contacts == 0) {
        printf(c31 "\n\n\tOops, pas encore de contacts à modifier !\n" c0);
        return;
    }
    ...
    while (1) {
        ...
        if (strlen(nouveau_nom) == 0 || verifier_alphanumerique(nouveau_nom)) {
            if (strlen(nouveau_nom) > 0) {
                strcpy(C->Nom[index], nouveau_nom);  // Si un nouveau nom a été entré, on le copie
            }
            break;
        } else {
            printf(c31 "\n\n\tErreur : Le nom saisi n'est pas valide. Veuillez réessayer.\n\n" c0);
    ...
    // Saisie du nouveau numéro de téléphone (avec validation)
    while (1) {
        printf("\n\tEntrez le nouveau numéro de téléphone (laisser vide pour garder l'ancien) : ");
        char nouveau_tel[20];
        ...
        if (strlen(nouveau_tel) == 0 || verifier_numero_tel(nouveau_tel)) {
            if (strlen(nouveau_tel) > 0) {
                strcpy(C->tel[index], nouveau_tel);  // Si un nouveau numéro a été entré, on le copie
            }
            break;
        } else {
            printf(c31 "\n\n\tErreur : Le numéro de téléphone saisi n'est pas valide. Veuillez réessayer.\n\n" c0);
    ...
    while (1) {
        ...
        if (strlen(nouvel_email) == 0 || isValidEmail(nouvel_email)) {
            if (strlen(nouvel_email) > 0) {
                strcpy(C->email[index], nouvel_email);  // Si un nouvel e-mail a été entré, on le copie
            }
            break;
        } else {
            printf(c31 "\n\n\tErreur : L'e-mail saisi n'est pas valide. Veuillez réessayer.\n\n" c0);
        }
    }
...
void supprimer_contact(Contact *C, int *nb_total_contacts) {
    if (*nb_total_contacts == 0) {
        printf(c31 "\n\n\tOops, pas encore de contacts à supprimer !\n" c0);
        return;
    }
    ...
    // Déplacer tous les contacts suivants d'une position pour combler le trou
    ...
    (*nb_total_contacts)--;  // Décrémenter le nombre total de contacts
    printf(c32 "\n\n\tLe contact a été supprimé avec succès.\n\n" c0);
}
void rechercher_contact(Contact C, int nb_total_contacts) {
    if (nb_total_contacts == 0) {
        printf(c31 "\n\n\tOops, pas encore de contacts à rechercher !\n" c0);
        return;
    }
    ...
    int trouve = 0;  // Flag pour savoir si le contact est trouvé
...
int main() {
    ...
    Contact C;          // Déclaration de la structure Contact
    ...
    do {
        ...
        switch (choix) {
            ...
            case 4: { 
                printf("\n\n\tAffichage des contacts ajoutés :\n");
